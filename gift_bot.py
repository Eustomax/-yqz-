...
import asyncio
import re
from send_reply import send_split_reply

# 管理员昵称（彩蛋/感谢文案中复用，原硬编码"卡米宝宝"）
ADMIN_NAME = "卡米宝宝"
...
def get_gift_reply(uid, uname):
    ...
    if uid not in MEMORY["gift"]:
        if uid == ADMIN_ID:
            return f"[礼物姬]{ADMIN_NAME}今天还没有送过礼物哦"
        if len(uname) > 23:
            uname = uname[:20] + "..."
    ...
    if uid == ADMIN_ID:
        return f"[礼物姬]{ADMIN_NAME}已送出{profit:.0f}电池的礼物！"
...
async def handle_thank_reply(uid, uname, reply):
    await send_split_reply(uid, reply, "[礼物姬]")
...
def thank_gift(uid, uname, gift_name, gift_value, cnt=1):
    ...
    if gift_name == "SuperChat":
        if len(uname) > 12:
            uname = uname[:9] + "..."
        return f"[礼物姬]哇！感谢{uname}老师送出的{gift_value/10:.0f}元SC！老板大气！" if uid != ADMIN_ID else f"[礼物姬]哇！感谢{ADMIN_NAME}送出的{gift_value/10:.0f}元SC！老板大气！"

    elif gift_name in ["舰长", "提督", "总督", "大航海"]:
        if len(uname) > 14: uname = uname[:11] + "..."
        if uid == ADMIN_ID and gift_name == "舰长" and cnt == 12:
            return f"[礼物姬]哇！感谢{ADMIN_NAME}的提督！老板大气！"
        elif cnt == 1:
            return f"[礼物姬]哇！感谢{uname}老师的{gift_name}！老板大气！" if uid != ADMIN_ID else f"[礼物姬]哇！感谢{ADMIN_NAME}的{gift_name}！老板大气！"
        else:
            if len(uname) > 12: uname = uname[:9] + "..."
            return f"[礼物姬]哇！感谢{uname}老师的{cnt}个月{gift_name}！老板大气！" if uid != ADMIN_ID else f"[礼物姬]哇！感谢{ADMIN_NAME}的{cnt}个月{gift_name}！老板大气！"

    elif match:
        guard = match.group(1)
        days = match.group(2)
        if len(uname) > 17: uname = uname[:14] + "..."
        return f"[礼物姬]哇！感谢{uname}老师的{days}天{guard}！老板大气！" if uid != ADMIN_ID else f"[礼物姬]哇！感谢{ADMIN_NAME}的{days}天{guard}！老板大气！"
        ...
        if len(uname) > 15:
            uname = uname[:12] + "..."
        return f"[礼物姬]哇！感谢{uname}老师投喂的{gift_name}！老板大气！" if uid != ADMIN_ID else f"[礼物姬]哇！感谢{ADMIN_NAME}投喂的{gift_name}！老板大气！"
...
async def call_gift(uid, uname):
    ...
    if uid == ADMIN_ID:
        add_log(f"[礼物姬] {ADMIN_NAME}触发礼物姬")
