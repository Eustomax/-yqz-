...
# 推进"你看又⭕️"档位的随机步长（电池数）——与 main_v2 / gift_bot 保持一致
...
def load_json_files():
    ...
    if os.path.exists(meta_path):
        try:
            with open(meta_path, "r", encoding="utf-8") as f:
                ...
                MEMORY["meta"]["next_threshold"] = data.get("next_threshold", random.randint(GEAR_STEP_MIN, GEAR_STEP_MAX))
        ...
        MEMORY["meta"]["next_threshold"] = random.randint(GEAR_STEP_MIN, GEAR_STEP_MAX)
        add_log(f"Next battery threshold: {MEMORY['meta']['next_threshold']}")
    ...
    for file_path, (key, target) in json_map.items():
        try:
            ...
            add_log(f"[ERROR] Error when reading {file_path}: {e}")
